...
class LagoupositonSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for page in range(int(self.totalPageCount)):
            page += 1
            yield scrapy.Request("{}&pn={}".format(response.url,page+1),callback=self.parse_id,meta={'kd':kd,'jresult':jresult},dont_filter=True)
    ...
    def parse_id(self, response):
        ...
        positionLink = "http://www.lagou.com/jobs"

        for each in jresult:
            print each['positionId']
            yield scrapy.Request("{}/{}.html".format(positionLink,each['positionId']), callback=self.parse_detail, meta={'kd': kd},dont_filter=True)
